...
func (s *OrderService) UpdateOrderStatus(ctx context.Context, profile external.Profile, orderID int, req models.OrderStatusRequest) error {
	...
	if req.Status == constants.OrderStatusRefund {
		...
		kafkaErr := s.External.ProduceKafkaMessage(ctx, helpers.GetEnv("KAFKA_TOPIC_REFUND", "refund-topic"), jsonPayload)
		if kafkaErr != nil {
			return errors.Wrap(kafkaErr, "failed to send refund message to kafka")
		}
	}
